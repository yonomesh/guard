...
//
// For example, a scope of "foo" returns modules named "foo.bar", "foo.loo", but not "bar", "foo.bar.loo", etc.
//
// An empty scope returns top-level modules, for example "foo" or "bar".
//
// Partial scopes are not matched (i.e. scope "foo.ba" does not match
// name "foo.bar").
//
